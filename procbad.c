...
scheduler(void)
{
  ...
  //  cprintf("[SCHEDULER] process[%d] on queue[%d]\n", procToBeScheduled->pid, procToBeScheduled->qno, c->apicid);
    curr[procToBeScheduled->pid]=procToBeScheduled->qno;
...
int cps()
{
  ...
  //  p->waittime = p->rtime - p->etime;
    ...
    if(p->state == 3)
    {
      cprintf("%d   %d        RUNNABLE    %d     %d     %d     %d    %d  %d  %d  %d  %d\n",p->pid,p->priority,p->rtime,p->waittime,p->num_r,curr[p->pid], p->ticks[0],p->ticks[1],p->ticks[2],p->ticks[3],p->ticks[4]);
      continue;
    }
    else if(p->state == 2)
    {
